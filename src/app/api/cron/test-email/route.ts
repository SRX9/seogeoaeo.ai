...
// CRON_SECRET so it can't be hit publicly. The recipient is hardcoded for this
// smoke test — in the real flow we resolve it from the DB (the workspace owner,
// via getWorkspaceOwnerEmail). Only works on the Cloudflare runtime where the
...
// TODO(test-only): in production the recipient comes from the DB, not a constant.
const TEST_RECIPIENT = "aquasr9@gmail.com";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: sent, to: TEST_RECIPIENT, from: EMAIL_FROM });
  } catch (error) {
    return NextResponse.json({
      ok: false,
      to: TEST_RECIPIENT,
