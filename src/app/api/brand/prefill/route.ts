  ...
  return handleApi(async () => {
    ...
    try {
      ...
      if (error instanceof RateLimitError) {
        throw new HttpError(429, "Too many prefill attempts — try again later", {
          code: "RATE_LIMITED",
        });
      }
      throw error;
    ...
    const details = await extractBrandDetails({ name, website: website || null });
    return jsonOk({ profile: details });
  });
}
