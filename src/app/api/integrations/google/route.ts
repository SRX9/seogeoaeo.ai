 ...
 * status, lists the user's GSC sites for the picker, saves the chosen site, and
 * disconnects. Proof is never metered.
 */
...
function emptyState(): SourceState {
  return { connected: false, siteUrl: null, lastSyncedAt: null, lastError: null };
}
...
export async function GET() {
  return handleApi(async () => {
    ...
    if (!brand) return jsonOk({ needsConnect: true, granted: false, sites: [], gsc: emptyState() });

    const connections = await listTrafficConnections(brand.id);
    const gsc = emptyState();
    for (const c of connections) {
      if (c.source !== "gsc") continue;
      gsc.connected = true;
      gsc.siteUrl = c.siteUrl;
      gsc.lastSyncedAt = c.lastSyncedAt?.toISOString() ?? null;
      gsc.lastError = c.lastError;
    }
    ...
    return jsonOk({ needsConnect: !granted, granted, sites, gsc });
  });
}
...
/** Save the chosen GSC site, then sync once so the panel fills. */
export async function POST(request: Request) {
  return handleApi(async () => {
    const { scope, session } = await requireApiBrand();
    const { siteUrl } = parseBody(saveSchema, await readJson(request));
