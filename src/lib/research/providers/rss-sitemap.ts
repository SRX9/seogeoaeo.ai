...
export const rssProvider: ResearchProvider = {
  ...
  async discover(context: ResearchContext) {
    const findingsByCompetitor = await Promise.all(
      context.competitors.flatMap((competitor) => {
        const rssUrl = competitor.rssUrl;
        if (!rssUrl) {
          return [];
        }
        return [
          (async (): Promise<ResearchFinding[]> => {
    ...
    return findingsByCompetitor.flat();
  },
};
...
export const sitemapProvider: ResearchProvider = {
  ...
  async discover(context: ResearchContext) {
    const findingsByCompetitor = await Promise.all(
      context.competitors.map(async (competitor): Promise<ResearchFinding[]> => {
        const sitemapUrl =
          competitor.sitemapUrl ?? `${competitor.url.replace(/\/$/, "")}/sitemap.xml`;
